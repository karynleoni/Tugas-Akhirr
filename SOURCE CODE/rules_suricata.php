<?php
	...
	$data = file_get_contents('/etc/suricata/rules/local.rules');
	...
	foreach($lines as $line)
	{
		...
		{				
			if($line[0] !== '#')
			{
				...
				$checkQuery = "SELECT idrules FROM suricatarules WHERE idrules = '$id_rules'";
				$result = $conn->query($checkQuery);

				if($result->num_rows == 0)
				{
					$query = "INSERT INTO suricatarules (action, protocol, ip_src, port_src, direction, ip_dest, port_dest, message, classtype, idrules) 
    ...
    <main>
		...
		<div class="par-add">
			...
			<div class="additional2">
				<?php
					$query = "SELECT COUNT(*) AS total_rows FROM suricatarules";
        ...
        <div class="monitor">
			<?php
				$query = "SELECT * FROM suricatarules";
